...
[colors,highlightColor,panelPos,parent,tabCardPVs,tabExtent,tabHeight,...
    tabLabelPVs,tabLabels,tabPos] = parseInputs(parent,tabLabels,varargin{:});
...
for tier = 1:nTiers
    for tabRank = 1:nTabs(tier)
        ...
        % 2014Feb24 P.D. => The following starting from rots = [90, 270]
        % is to create vertical text as a picture and set it as cdata
        % into tab
        % Let's try to skip this procedure if 'rotation' = 0
        if ~isempty(tabLabelPVs)
            indrot = find(strcmpi('rotation',tabLabelPVs));
            if  ~isempty(indrot)
                v_rot = tabLabelPVs(indrot + 1);
                tabLabelPVs(indrot:indrot + 1) = [];
            end
        end
        
        if ismember(tabPos,{'t','b'}) || v_rot{1} == 0
...
% BEGIN NESTED FUNCTIONS
    ...
    function [colors,highlightColor,panelPos,parent,tabCardPVs,tabExtent,...
            tabHeight,tabLabelPVs,tabLabels,tabPos]...
            = parseInputs(parent,tabLabels,varargin)
        % DEFAULT Colors
        colors = jet(totalTabs+1);
...
% VALIDATE TAB LABELS
    ...
    function rectPosn = calculatePositions
        ...
        for tier = 1:nTiers
            ...
            elseif strcmp(tabPos,'l')
                ...
                % new same as for 'Right' to have first tab top most
                rectPosn{tier}(:,2)=1-((1:nTabs(tier))*tabLength(tier));
                % Original to have first tab lowest most
    ...
    function toggleCardVisibility(tier,tabRank,visibility,varargin)
        if ischar(tier)% tier = 'all'
            ...
            % P.D. add-hock way to highlight selected panel and tab
            % Think to add this color to PVs
                    set(allHandles(ii),'backgroundcolor',colors(ii,:))
                    set(tabCardHandles{tier}(ii),'backgroundcolor',colors(ii,:))
                end
            end
        else
            ...
            % P.D. add-hock way to highlight selected panel and tab
            % Think to add this color to PVs
            set(tabCardHandles{tier}(tabRank),'backgroundcolor',[0.75 0.75 0.75])
            set(tabHandles{1}(tabRank),'backgroundcolor',[0.75 0.75 0.75])
        end
    end
